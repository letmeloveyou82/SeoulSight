...
## 📝 회고
- MySQL에서 Elasticsearch로 데이터를 파싱할 때, 컬럼의 순서가 유지되지 않았습니다. 이로 인해 다른 인덱싱 방식을 사용한다는 것을 알 수 있었습니다. 또한, logstash에서 데이터 covert 설정을 하지 않을 시, 시각화의 데이터로 활용할 수 없음을 경험했습니다. ES 스택을 활용하면서 특징을 파악할 수 있었습니다.
